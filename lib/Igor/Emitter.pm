...
Using the L<Igor::Event> class, subscribers can stop an event from being
...
By calling L<stop_default|Igor::Event/stop_default>, we set the
L<is_default_stopped|Igor::Event/is_default_stopped> flag, which the door sees
and decides not to open.

=head2 Using Custom Events

The default C<Igor::Event> is really only useful for notifications. If you want
...
Now that we can represent who is knocking, let's notify our subscribers.
    ...
    package Door;
    use Moo;
    use Door::Knock; # Our emitter must load the class, Igor::Emitter will not
    with 'Igor::Emitter';
...
=head2 Without Igor::Event

Although checking C<is_default_stopped> is completely optional, if you do not
wish to use the C<Igor::Event> object, you can instead call L<emit_args>
...
Coordinating Christmas Dinner with Igor::Emitter by Yanick Champoux.
...
# EVENT_NAME => [ Igor::Listener object, ... ]
...
C<subref> is a subroutine reference that will get either a L<Igor::Event> object
...
object of class L<Igor::Listener>.  If more information should be
stored, create a custom subclass of L<Igor::Listener> and use C<%args>
...
pairs to give to the C<Igor::Event> constructor.
...
sub emit {
    ...
    my $class = delete $args{ class } || "Igor::Event";
...
Use this if you want to avoid using L<Igor::Event>, though you miss out on the control
features like L<stop|Igor::Event/stop> and L<stop default|Igor::Event/stop_default>.
...
instances of L<Igor::Listener> or of custom classes specified in calls
to L</subscribe>.
