...
=head1 SYNOPSIS
    ...
    # script.pl
    use Igor;
    my $wire = Igor->new( file => 'wire.yml' );
    my $captain = $wire->get( 'captain' );
    print $captain->name; # "Malcolm Reynolds"

=head1 DESCRIPTION

Igor is a configuration module and a dependency injection
container. In addition to complex data structures, Igor configures
...
Using Igor in your application brings great flexibility,
allowing users to easily add their own code to customize how your
project behaves.

For an L<introduction to the Igor service configuration format,
see Igor::Help::Config|Igor::Help::Config>.
...
use Data::Dumper;
use Igor::Event::ConfigService;
use Igor::Event::BuildService;
use constant DEBUG => $ENV{IGOR_DEBUG};
with 'Igor::Emitter';
...
configurations|Igor::Help::Config>.
...
See L<Igor::Help::Config for details on what the configuration
data structure looks like|Igor::Help::Config>.
    ...
    if ( $name =~ q{/} ) {
        ...
        my $unsub_config = $container->on( configure_service => sub {
            ...
            $self->emit( configure_service =>
                class => 'Igor::Event::ConfigService',
...
and C<args>. An object of any class can be created with Igor.
...
If the C<class> consumes the L<Igor::Service role|Igor::Service>,
...
examples of this|Igor::Help::Config/Multiple Constructor
Methods>).
...
Attach an event handler to a L<Igor::Emitter subclass|Igor::Emitter>. This
...
sub create_service {
    ...
    if ( exists $service_info{value} && (
        ...
        Igor::Exception::InvalidConfig->throw(
            name => $name,
            file => $self->file,
    ...
    if ( ref $method eq 'ARRAY' ) {
        ...
        if ( $service_info{class}->can( 'DOES' ) && $service_info{class}->DOES( 'Igor::Service' ) ) {
...
Igor service. This is used to identify service configuration
...
# Intended for use with igor script
sub validate {
    my $error_count = 0;
...
If there is an error internal to Igor, an exception will be thrown. If there is an
error with creating a service or calling a method, the exception thrown will be passed-
through unaltered.

=head2 Igor::Exception
...
=head2 Igor::Exception::Config

An exception loading the configuration file.

=cut

package Igor::Exception::Config;
use Moo;
use Types::Standard qw( :all );
extends 'Igor::Exception';
...
package Igor::Exception::Service;
use Moo;
use Types::Standard qw( :all );
extends 'Igor::Exception';
...
=head2 Igor::Exception::NotFound

The requested service or configuration was not found.

=cut

package Igor::Exception::NotFound;
use Moo;
extends 'Igor::Exception::Service';
...
=head2 Igor::Exception::InvalidConfig
...
package Igor::Exception::InvalidConfig;
use Moo;
extends 'Igor::Exception::Service';
use overload q{""} => sub {
    my ( $self ) = @_;
...
Event handlers get a L<Igor::Event::ConfigService> object as their
only argument.
...
Event handlers get a L<Igor::Event::BuildService> object as their
only argument.
